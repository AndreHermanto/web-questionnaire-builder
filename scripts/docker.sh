...
if [ "$PART_OF_BRANCH_NAME" == "develop" ] || [ "$PART_OF_BRANCH_NAME" == "uat" ] || [ "$PART_OF_BRANCH_NAME" == "master" ]; then
        ...
        if [ "$PART_OF_BRANCH_NAME" == "uat" ]; then
            REACT_APP_GATRACKID=UA-81359825-2 REACT_APP_BASE_URL=/api/gw-$clean_app_name/v1 CI=true npm run build 2> >(tee build.txt >&2)
        elif [ "$PART_OF_BRANCH_NAME" == "master" ]; then
            REACT_APP_GATRACKID=UA-81359825-3 REACT_APP_BASE_URL=/api/gw-$clean_app_name/v1 CI=true npm run build 2> >(tee build.txt >&2)
        else
            REACT_APP_BASE_URL=/api/gw-$clean_app_name/v1 CI=true npm run build 2> >(tee build.txt >&2)
        fi
