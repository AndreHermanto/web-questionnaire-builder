...
Then(/^I (?:wait).*? to (?:find|identify|see|spot).*? (`.*`).*?$/, selectorChain => client
  .useCss()
...
When(/^I (?:enter|input|supply|type).*? "(.*?)" in.*? (`.*`)$/, (value, selectorChain) => client
  .useCss()
  .waitForElementVisible(createNestedSelector(selectorChain), defaultWaitTime)
  .setValue(createNestedSelector(selectorChain), value));

When(/^I (?:check).*? (`.*`) not empty$/, value => client.useCss().getText(createNestedSelector(value), (result) => {
  this.assert.notEqual('', result.value);
}));

When(/^I (?:activate|click).*? (`.*`)$/, selectorChain => client
  .useCss()
  .waitForElementVisible(createNestedSelector(selectorChain), defaultWaitTime)
